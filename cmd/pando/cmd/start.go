...
import (
	...
	"github.com/pandotoken/pando/node"
	msg "github.com/pandotoken/pando/p2p/messenger"
	msgl "github.com/pandotoken/pando/p2pl/messenger"
...
func loadOrCreateKey() (*crypto.PrivateKey, error) {
	...
	if numAddrs == 0 {
		if len(nodePassword) != 0 {
			...
			fmt.Println("")
			fmt.Println("You are launching the Pando Node for the first time. Welcome and please follow the instructions to setup the node.")
			fmt.Println("")

			firstPrompt := fmt.Sprintf("Please choose your password for the Pando Node: ")
			...
			fmt.Println("")
			fmt.Println("-----------------------------------------------------------------------------------------------------")
			fmt.Println("IMPORTANT: Please store your password securely. You will need it each time you launch the Pando node.")
			fmt.Println("-----------------------------------------------------------------------------------------------------")
			fmt.Println("")
		...
		prompt := fmt.Sprintf("Please enter the password to launch the Pando node: ")
		if len(nodePassword) != 0 {
			password = nodePassword
		} else {
...
func printCountdown() {
	for i := 10; i >= 0; i-- {
		fmt.Printf("\rLaunching Pando to da moon: %d...", i)
