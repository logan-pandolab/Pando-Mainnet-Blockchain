...
func doStakeReturnsCmd(cmd *cobra.Command, args []string) {
	...
	var err error
	if height == 0 {
		res, err = client.Call("pando.GetAllPendingEliteEdgeNodeStakeReturns", rpc.GetAllPendingEliteEdgeNodeStakeReturnsArgs{})
	} else {
		res, err = client.Call("pando.GetEliteEdgeNodeStakeReturnsByHeight", rpc.GetEliteEdgeNodeStakeReturnsByHeightArgs{Height: common.JSONUint64(height)})
	}
	if err != nil {
