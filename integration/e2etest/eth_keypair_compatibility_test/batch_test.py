...
def GetInitialFaucetSeq():
  query_cmd = 'pandocli query account --address=%s'%(FAUCET_ADDRESS)
...
def BatchTest(init_faucet_seq):
  ...
  for idx in range(500):
    ...
    os.system("pandocli query account --address=%s"%(address))
    print("----------------------------------------------------------------")
    print("")
...
# __MAIN__
#
# Before running this script, we need to launch both the `pando` and `pandocli` daemon connected 
