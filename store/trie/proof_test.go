...
import (
	...
	dbbackend "github.com/pandotoken/pando/store/database/backend"
)
...
// test all variations.
func makeProvers(trie *Trie) []func(key []byte) *dbbackend.MemDatabase {
	var provers []func(key []byte) *dbbackend.MemDatabase

	// Create a direct trie based Merkle prover
	provers = append(provers, func(key []byte) *dbbackend.MemDatabase {
		proof := dbbackend.NewMemDatabase()
...
func BenchmarkProve(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		proofs := dbbackend.NewMemDatabase()
...
func BenchmarkVerifyProof(b *testing.B) {
	...
	root := trie.Hash()
	var keys []string
	var proofs []*dbbackend.MemDatabase
	for k := range vals {
		keys = append(keys, k)
		proof := dbbackend.NewMemDatabase()
