...
import (
	...
	"github.com/pandotoken/pando/store/trie"
)
...
type BlockHeader struct {
	...
	GuardianVotes      *AggregatedVotes    `rlp:"nil"` // Added in Pando2.0 fork.
	EliteEdgeNodeVotes *AggregatedEENVotes `rlp:"nil"` // Added in Pando3.0 fork.
	TxHash             common.Hash
...
func (h *BlockHeader) EncodeRLP(w io.Writer) error {
	...
	if h.Height < common.HeightEnablePando2 {
		return rlp.Encode(w, []interface{}{
			h.ChainID,
			h.Epoch,
			h.Height,
	...
	// Pando2.0 fork
	if h.Height >= common.HeightEnablePando2 && h.Height < common.HeightEnablePando3 {
		return rlp.Encode(w, []interface{}{
			h.ChainID,
			h.Epoch,
			h.Height,
...
func (h *BlockHeader) DecodeRLP(stream *rlp.Stream) error {
	...
	// Pando3.0 fork
	if h.Height >= common.HeightEnablePando3 {
		raw, err := stream.Raw()
		if err != nil {
			return err
		}
