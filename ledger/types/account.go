...
func (acc *Account) UpdateToHeight(height uint64) {
	//	acc.UpdateAccountPTXReward(height) // Initial PTX inflation should be zero for all accounts
	acc.ReleaseExpiredFunds(height)
}

// func (acc *Account) UpdateAccountPTXReward(currentBlockHeight uint64) {
...
// 	totalPandoWei := acc.Balance.PandoWei
// 	if totalPandoWei == nil {
// 		totalPandoWei = big.NewInt(0)
// 	}
// 	span := currentBlockHeight - acc.LastUpdatedBlockHeight

// 	newPTXBalance := big.NewInt(int64(span))
// 	newPTXBalance.Mul(newPTXBalance, totalPandoWei)
// 	newPTXBalance.Mul(newPTXBalance, big.NewInt(RegularPTXGenerationRateNumerator))
// 	newPTXBalance.Div(newPTXBalance, big.NewInt(RegularPTXGenerationRateDenominator))

// 	if newPTXBalance.Sign() <= 0 {
// 		// Underflow, no reward to add yet
// 		return
// 	}

// 	newPTXBalance.Add(newPTXBalance, acc.Balance.PTXWei)

// 	if !newPTXBalance.IsInt64() {
// 		panic("Account PTX balance will overflow")
// 	}

// 	newBalance := Coins{
// 		PandoWei: acc.Balance.PandoWei,
// 		PTXWei: newPTXBalance,
// 	}
