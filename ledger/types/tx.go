...
func (tx *RametronStakeTx) SetSignature(addr common.Address, sig *crypto.Signature) bool {
	for i, input := range tx.Inputs {
		if input.Address == addr {
			tx.Inputs[i].Signature = sig
			return true
...
func (tx *SmartContractTx) EthSigningHash(chainID string, blockHeight uint64) common.Hash {
	...
	ethSigningHash := RLPHash([]interface{}{
		tx.From.Sequence - 1, // off-by-one, ETH tx nonce starts from 0, while Pando tx sequence starts from 1
		tx.GasPrice,
		tx.GasLimit,
		toAddress,
		tx.From.Coins.NoNil().PTXWei,
		tx.Data,
...
func (tx *SmartContractTx) String() string {
	return fmt.Sprintf("SmartContractTx{%v -> %v, value: %v, gas_limit: %v, gas_price: %v, data: %v}",
		tx.From.Address.Hex(), tx.To.Address.Hex(), tx.From.Coins.PTXWei, tx.GasLimit, tx.GasPrice, tx.Data)
}
...
func (tx *DepositStakeTx) String() string {
	return fmt.Sprintf("DepositStakeTx{%v -> %v, stake: %v, purpose: %v}",
		tx.Source.Address, tx.Holder.Address, tx.Source.Coins.PandoWei, tx.Purpose)
}
...
func (tx *DepositStakeTxV2) String() string {
	return fmt.Sprintf("DepositStakeTxV2{%v -> %v, stake: %v, purpose: %v, BlsPubkey: %v, BlsPop: %v}",
		tx.Source.Address, tx.Holder.Address, tx.Source.Coins.PandoWei, tx.Purpose, tx.BlsPubkey, tx.BlsPop)
}
...
// It allows the stake holder to specify a "beneficiary" to receive a fraction of the Pando/PTX staking reward. The split fraction
