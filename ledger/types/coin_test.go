...
func TestInvalidCoin(t *testing.T) {
	...
	ret1.PandoWei = big.NewInt(456)
	assert.True(coinsA.PandoWei.Cmp(big.NewInt(123)) == 0)
	assert.True(ret1.PandoWei.Cmp(big.NewInt(456)) == 0)

	ret2 := coinsA.Minus(coinsEmpty)
	assert.True(coinsA.IsEqual(ret2), "Sum is correct")
	// Result should be a copy
	ret2.PandoWei = big.NewInt(456)
	assert.True(coinsA.PandoWei.Cmp(big.NewInt(123)) == 0)
	assert.True(ret2.PandoWei.Cmp(big.NewInt(456)) == 0)
}
...
func TestcCoinsRLPNil(t *testing.T) {
	...
	assert.NotNil(b.PandoWei)
	assert.NotNil(b.PTXWei)
...
func TestNoNilException(t *testing.T) {
	...
	coinsD := coinsB.NoNil()
	assert.Equal(int64(0), coinsD.PandoWei.Int64())
	assert.Equal(int64(0), coinsD.PTXWei.Int64())
}
...
func TestJSON(t *testing.T) {
	...
	assert.Equal(0, num.Cmp(d.PandoWei))
	assert.Nil(d.PTXWei)
}
