...
import (
	...
	"github.com/pandotoken/pando/store/treestore"
)
...
func (sv *StoreView) SubBalance(addr common.Address, amount *big.Int) {
	...
	account.Balance.PTXWei.Sub(account.Balance.PTXWei, amount)
	sv.SetAccount(addr, account)
...
func (sv *StoreView) AddBalance(addr common.Address, amount *big.Int) {
	...
	account.Balance = account.Balance.NoNil()
	account.Balance.PTXWei.Add(account.Balance.PTXWei, amount)
	sv.SetAccount(addr, account)
...
func (sv *StoreView) GetBalance(addr common.Address) *big.Int {
	return sv.GetOrCreateAccount(addr).Balance.PTXWei
}

func (sv *StoreView) SubPandoBalance(addr common.Address, amount *big.Int) {
	if amount.Sign() == 0 {
		return
	}
	account := sv.GetAccount(addr)
	...
	account.Balance = account.Balance.NoNil()
	account.Balance.PandoWei.Sub(account.Balance.PandoWei, amount)
	sv.SetAccount(addr, account)
...
func (sv *StoreView) AddPandoBalance(addr common.Address, amount *big.Int) {
	if amount.Sign() == 0 {
		return
	}
	account := sv.GetOrCreateAccount(addr)
	account.Balance = account.Balance.NoNil()
	account.Balance.PandoWei.Add(account.Balance.PandoWei, amount)
	sv.SetAccount(addr, account)
...
// GetPandoBalance returns the PandoWei balance of the given address
func (sv *StoreView) GetPandoBalance(addr common.Address) *big.Int {
	return sv.GetOrCreateAccount(addr).Balance.PandoWei
}

// GetPandoStake returns the total amount of PandoWei the address staked to validators and/or guardians
func (sv *StoreView) GetPandoStake(addr common.Address) *big.Int {
...
func (sv *StoreView) Suicide(addr common.Address) bool {
	...
	sv.addBalanceChange(&types.BalanceChange{
		...
		TokenType:  1,
		IsNegative: true,
		Delta:      new(big.Int).Set(account.Balance.PTXWei),
	})

	account.Balance.PTXWei = big.NewInt(0)
	sv.SetAccount(addr, account)
	return true
}
