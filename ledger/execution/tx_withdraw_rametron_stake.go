...
var _ TxExecutor = (*WithdrawRametronStakeTxExecutor)(nil)
...
func (exec *WithdrawRametronStakeTxExecutor) sanityCheck(chainID string, view *st.StoreView, transaction types.Tx) result.Result {
	...
	if len(tx.Inputs) == 0 || len(tx.Outputs) == 0 {
		return result.Error("Invalid withdrawrametronStakeTx, Inputs and/or Outputs are empty")
	}
	...
	if blockHeight >= common.HeightEnableSmartContract {
		for _, outAcc := range accounts {
			if outAcc.IsASmartContract() {
				return result.Error(
					fmt.Sprintf("Sending Pando/PTX to a smart contract (%v) through a WithdrawRametronStakeTx transaction is not allowed", outAcc.Address))
			}
		}
	}
...
func (exec *WithdrawRametronStakeTxExecutor) getTxInfo(transaction types.Tx) *core.TxInfo {
	tx := transaction.(*types.WithdrawRametronStakeTx)
	return &core.TxInfo{
...
func (exec *WithdrawRametronStakeTxExecutor) calculateEffectiveGasPrice(transaction types.Tx) *big.Int {
	tx := transaction.(*types.WithdrawRametronStakeTx)
	fee := tx.Fee
	numAccountsAffected := uint64(len(tx.Inputs) + len(tx.Outputs))
