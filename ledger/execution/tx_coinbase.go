...
var eenPTXRewardPerBlock = big.NewInt(1).Mul(big.NewInt(38), weiMultiplier) // 38 PTX per block, corresponds to about 4% *initial* annual inflation rate. The inflation rate naturally approaches 0 as the chain grows.
...
func RetrievePools(ledger core.Ledger, chain *blockchain.Chain, db database.Database, blockHeight uint64, guardianVotes *core.AggregatedVotes,
	...
	if blockHeight < common.HeightEnablePando2 {
		guardianPool = nil
		eliteEdgeNodePool = nil
	} else if blockHeight < common.HeightEnablePando3 {
	...
	} else { // blockHeight >= common.HeightEnablePando3
...
func CalculateReward(ledger core.Ledger, view *st.StoreView, validatorSet *core.ValidatorSet,
	...
	} else if blockHeight < common.HeightEnablePando2 || guardianVotes == nil || guardianPool == nil {
...
// grant uptime mining rewards to active elite edge nodes (they are the ptx stakers)
func grantEliteEdgeNodeReward(ledger core.Ledger, view *st.StoreView, guardianVotes *core.AggregatedVotes, eliteEdgeNodeVotes *core.AggregatedEENVotes,
	...
	// the source of the stake divides the block reward proportional to their stake
	totalReward := big.NewInt(1).Mul(eenPTXRewardPerBlock, big.NewInt(common.CheckpointInterval))
