...
func (exec *DepositStakeExecutor) sanityCheck(chainID string, view *st.StoreView, viewSel core.ViewSelector, transaction types.Tx) result.Result {
	...
	if _, ok := transaction.(*types.DepositStakeTxV2); ok && blockHeight < common.HeightEnablePando2 {
...
//	if (tx.Purpose == core.StakeForValidator || tx.Purpose == core.StakeForGuardian) && stake.PTXWei.Cmp(types.Zero) != 0 {
//		return result.Error("PTX has to be zero for validator or guardian stake deposit!").
//			WithErrorCode(result.CodeInvalidStake)
//	}

	if tx.Purpose == core.StakeForEliteEdgeNode && stake.PandoWei.Cmp(types.Zero) != 0 {
		return result.Error("Pando has to be zero for elite edge node stake deposit!").
			WithErrorCode(result.CodeInvalidStake)
	}
	...
	if tx.Purpose == core.StakeForEliteEdgeNode {
		if blockHeight < common.HeightEnablePando3 {
			return result.Error(fmt.Sprintf("Elite Edge Node staking not enabled yet, please wait until block height %v", common.HeightEnablePando3)).WithErrorCode(result.CodeGenericError)
		}

		minEliteEdgeNodeStake := core.MinEliteEdgeNodeStakeDeposit
		maxEliteEdgeNodeStake := core.MaxEliteEdgeNodeStakeDeposit

		if stake.PTXWei.Cmp(big.NewInt(0)) > 0 {
			return result.Error("Only PTX can be deposited for elite edge nodes").
				WithErrorCode(result.CodeStakeExceedsCap)
		}

		if stake.PTXWei.Cmp(minEliteEdgeNodeStake) < 0 {
			return result.Error("Insufficient amount of stake, at least %v PTXWei is required for each elite edge node deposit", minEliteEdgeNodeStake).
				WithErrorCode(result.CodeInsufficientStake)
		}
		...
		if expectedStake.Cmp(maxEliteEdgeNodeStake) > 0 {
			return result.Error("Stake exceeds the cap, at most %v PTXWei can be deposited to each elite edge node", maxEliteEdgeNodeStake).
				WithErrorCode(result.CodeStakeExceedsCap)
		}
	}
...
func (exec *DepositStakeExecutor) process(chainID string, view *st.StoreView, viewSel core.ViewSelector, transaction types.Tx) (common.Hash, result.Result) {
	...
	if tx.Purpose == core.StakeForValidator {
		...
		sourceAccount.Balance = sourceAccount.Balance.Minus(stake)
		stakeAmount := stake.PTXWei // elite edge node deposits PTX
