...
type Executor struct {
	...
	withdrawrametronStakeTxExec   *WithdrawRametronStakeTxExecutor
...
func NewExecutor(db database.Database, chain *blockchain.Chain, state *st.LedgerState, consensus core.ConsensusEngine, valMgr core.ValidatorManager, ledger core.Ledger) *Executor {
	executor := &Executor{
		...
		rametronStakeTxExec:           NewRametronStakeTxExecutor(state),
		withdrawrametronStakeTxExec:   NewWithdrawRametronStakeTxExecutor(state),
...
func (exec *Executor) getTxExecutor(tx types.Tx) TxExecutor {
	...
	case *types.WithdrawRametronStakeTx:
		txExecutor = exec.withdrawrametronStakeTxExec
