...
func NewRametronStakeTxExecutor(state *st.LedgerState) *RametronStakeTxExecutor {
	return &RametronStakeTxExecutor{
		state: state,
	}
}

func (exec *RametronStakeTxExecutor) sanityCheck(chainID string, view *st.StoreView, viewSel core.ViewSelector, transaction types.Tx) result.Result {
...
func (exec *RametronStakeTxExecutor) process(chainID string, view *st.StoreView, viewSel core.ViewSelector, transaction types.Tx) (common.Hash, result.Result) {
	tx := transaction.(*types.RametronStakeTx)
