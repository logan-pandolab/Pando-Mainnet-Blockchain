...
func Execute(parentBlock *core.Block, tx *types.SmartContractTx, storeView *state.StoreView) (evmRet common.Bytes,
	...
	value := tx.From.Coins.PTXWei
	if value == nil {
		value = big.NewInt(0)
	}

	pandoValue := tx.From.Coins.PandoWei
	if pandoValue == nil {
	...
	if createContract {
		code := tx.Data
		evmRet, contractAddr, leftOverGas, evmErr = evm.Create(AccountRef(fromAddr), code, remainingGas, value, pandoValue)
	} else {
		input := tx.Data
		evmRet, leftOverGas, evmErr = evm.Call(AccountRef(fromAddr), contractAddr, input, remainingGas, value, pandoValue)
	}
