...
import (
	...
	"github.com/pandotoken/pando/crypto/bn256"
...
var PrecompiledContractsByzantium = map[common.Address]PrecompiledContract{
	...
	common.BytesToAddress([]byte{201}): &pandoBalance{},
	common.BytesToAddress([]byte{202}): &pandoStake{},
}

var PrecompiledContractsPandoSupport = map[common.Address]PrecompiledContract{
	common.BytesToAddress([]byte{1}): &ecrecover{},
	common.BytesToAddress([]byte{2}): &sha256hash{},
	common.BytesToAddress([]byte{3}): &ripemd160hash{},
	common.BytesToAddress([]byte{4}): &dataCopy{},
	...
	common.BytesToAddress([]byte{201}): &pandoBalance{},
	common.BytesToAddress([]byte{202}): &pandoStake{},
	common.BytesToAddress([]byte{203}): &transferPando{},
}
...
// pandoBalance retrieves the PandoWei balance of the given address
type pandoBalance struct {
}

// RequiredGas returns the gas required to execute the pre-compiled contract.
func (c *pandoBalance) RequiredGas(input []byte, blockHeight uint64) uint64 {
	return params.PandoBalanceGas
}

func (c *pandoBalance) Run(evm *EVM, input []byte, callerAddr common.Address) ([]byte, error) {
	address := common.BytesToAddress(input)
	pandoBalance := evm.StateDB.GetPandoBalance(address)
	pandoBalanceBytes := pandoBalance.Bytes()
	pandoBalanceBytes32 := common.LeftPadBytes(pandoBalanceBytes[:], 32) // easier to convert bytes32 into uint256 in smart contracts
	return pandoBalanceBytes32, nil
}

// pandoStake retrieves the total amount of PandoWei the address staked to validators and/or guardians
type pandoStake struct {
}

// RequiredGas returns the gas required to execute the pre-compiled contract.
func (c *pandoStake) RequiredGas(input []byte, blockHeight uint64) uint64 {
	return params.PandoStakeGas
}

func (c *pandoStake) Run(evm *EVM, input []byte, callerAddr common.Address) ([]byte, error) {
	address := common.BytesToAddress(input)
	pandoStake := evm.StateDB.GetPandoStake(address)
	pandoStakeBytes := pandoStake.Bytes()
	pandoStakeBytes32 := common.LeftPadBytes(pandoStakeBytes[:], 32) // easier to convert bytes32 into uint256 in smart contracts
	return pandoStakeBytes32, nil
}

// transferPando transfers the Pando token
type transferPando struct {
}

// RequiredGas returns the gas required to execute the pre-compiled contract.
func (c *transferPando) RequiredGas(input []byte, blockHeight uint64) uint64 {
	return params.PandoTransferGas
}

func (c *transferPando) Run(evm *EVM, input []byte, callerAddr common.Address) ([]byte, error) {
	recipient := common.BytesToAddress(getData(input, 0, 20))
	pandoWeiAmount := new(big.Int).SetBytes(getData(input, 20, 32))
	if !CanTransferPando(evm.StateDB, callerAddr, pandoWeiAmount) {
		return common.Bytes{}, ErrInsufficientPandoBlance
	}

	// send Pando from the contract to the specified recipient
	TransferPando(evm.StateDB, callerAddr, recipient, pandoWeiAmount)
