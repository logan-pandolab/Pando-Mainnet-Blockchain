...
func opCreate(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	...
	contract.UseGas(gas)
	pandoValue := big.NewInt(0)
	res, addr, returnGas, suberr := interpreter.evm.Create(contract, input, gas, value, pandoValue)
...
func opCreate2(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	...
	gas -= gas / 64
	contract.UseGas(gas)
	pandoEndowment := big.NewInt(0)
	res, addr, returnGas, suberr := interpreter.evm.Create2(contract, input, gas, endowment, pandoEndowment, salt)
...
func opCall(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	...
	pandoValue := big.NewInt(0)
	ret, returnGas, err := interpreter.evm.Call(contract, toAddr, args, gas, value, pandoValue)
	if err != nil {
		stack.push(interpreter.intPool.getZero())
	} else {
