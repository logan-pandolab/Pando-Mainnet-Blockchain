...
import (
	...
	exec "github.com/pandotoken/pando/ledger/execution"
	...
	p2psim "github.com/pandotoken/pando/p2p/simulation"
...
func genSimSnapshot(chainID string, db database.Database) (snapshot mockSnapshot, srcPrivAccs []*types.PrivAccount, valPrivAccs []*types.PrivAccount) {
	...
	src1Acc.Balance = types.Coins{
		PandoWei: new(big.Int).Mul(new(big.Int).SetUint64(20), core.MinValidatorStakeDeposit),
		PTXWei:   new(big.Int).Mul(new(big.Int).SetUint64(100), core.MinValidatorStakeDeposit),
	}
...
func newRawRametronStakeTx(chainID string, sequence int, addPubKey bool, accOut, accIn types.PrivAccount, injectFeeFluctuation bool) common.Bytes {
	delta := int64(0)
	var err error
	if injectFeeFluctuation {
		// inject so fluctuation into the txFee, so later we can test whether the
	...
	inAccs := []types.PrivAccount{accIn}
	for idx, in := range rametronStakeTx.Inputs {
		inAcc := inAccs[idx]
		sig, err := inAcc.PrivKey.Sign(signBytes)
		if err != nil {
			panic("Failed to sign the coinbase transaction")
		}
		rametronStakeTx.SetSignature(in.Address, sig)
	}

	sendTxBytes, err := types.TxToBytes(rametronStakeTx)
	if err != nil {
		panic(err)
	}
	return sendTxBytes
