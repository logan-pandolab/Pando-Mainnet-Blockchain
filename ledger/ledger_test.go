...
func TestLedgerProposerBlockTxs(t *testing.T) {
	...
	// Transaction sanity checks
	var prevRametronStakeTx *types.RametronStakeTx
	for idx := 0; idx < expectedTotalNumTx; idx++ {
		rawTx := blockTxs[idx]
		tx, err := types.TxFromBytes(rawTx)
		assert.Nil(err)
		...
		case *types.RametronStakeTx:
			assert.True(idx >= 0)
			currRametronStakeTx := tx.(*types.RametronStakeTx)
			if prevRametronStakeTx != nil {
				// mempool should works like a priority queue, for the same type of tx (i.e. RametronStakeTx),
				// those with higher fee should get reaped first
				feeDiff := prevRametronStakeTx.Fee.Minus(currRametronStakeTx.Fee)
				assert.True(feeDiff.IsNonnegative())
...
func TestLedgerApplyBlockTxs(t *testing.T) {
	...
	inAccInitPTXWei := accIns[0].Balance.PTXWei
...
func TestValidatorStakeUpdate(t *testing.T) {
	...
	assert.True(returnedCoins.PandoWei.Cmp(new(big.Int).Mul(new(big.Int).SetUint64(5), core.MinValidatorStakeDeposit)) == 0)
...
func TestGuardianStakeUpdate(t *testing.T) {
	...
	// The 1st deposit(1*minimal) was from holder to holder , 2nd deposit(2*minimal) and 3rd deposit
	// (3*minimal)was from source to holder.
	assert.Equal(0, returnedCoins.PandoWei.Cmp(new(big.Int).Mul(new(big.Int).SetUint64(5), core.MinGuardianStakeDeposit)))
	assert.True(returnedCoins.PTXWei.Cmp(core.Zero) == 0)
	log.Infof("Returned coins: %v", returnedCoins)
}
