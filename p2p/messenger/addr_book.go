...
import (
	...
	mm "github.com/pandotoken/pando/common/math"
...
type AddrBook struct {
	...
	rand              *rand.Rand
	key               string
	ourAddrs          map[string]*nu.NetAddress
...
func (a *AddrBook) GetSelection() []*nu.NetAddress {
	...
	allAddr := make([]*nu.NetAddress, a.size())
	i := 0
	...
	numAddresses := mm.MaxInt(
		mm.MinInt(minGetSelection, len(allAddr)),
		len(allAddr)*getSelectionPercent/100)
	numAddresses = mm.MinInt(maxGetSelection, numAddresses)
...
func newKnownAddress(addr *nu.NetAddress, src *nu.NetAddress) *knownAddress {
	return &knownAddress{
		Addr:        addr,
		Src:         src,
		Attempts:    0,
