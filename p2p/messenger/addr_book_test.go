...
import (
	...
	"github.com/pandotoken/pando/p2p/netutil"
)
...
type netAddressPair struct {
	addr *netutil.NetAddress
	src  *netutil.NetAddress
}
...
func randIPv4Address(t *testing.T) *netutil.NetAddress {
	for {
		...
		addr, err := netutil.NewNetAddressString(fmt.Sprintf("%v:%v", ip, port))
