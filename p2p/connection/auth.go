...
import (
	...
	"github.com/pandotoken/pando/crypto/ecies"
	"github.com/pandotoken/pando/crypto/secp256k1"
	"github.com/pandotoken/pando/crypto/sha3"
...
func (rw *rlpxFrameRW) WritePacket(packet *Packet) error {
	...
	if _, err := io.Copy(tee, bytes.NewReader(raw)); err != nil {
		return err
	}
...
func (rw *rlpxFrameRW) ReadPacket() (*Packet, error) {
	// read the header
	headbuf := make([]byte, 32)
	...
	packet := &Packet{}
	if err := rlp.Decode(content, &packet); err != nil {
		return nil, err
	}
	return packet, nil
}
